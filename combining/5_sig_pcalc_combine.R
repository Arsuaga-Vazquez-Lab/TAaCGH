...
{
	chr1 <- chrDict[i,1];
	arm1 <- as.vector(chrDict[i,2]);
	chrArm1 <- paste(chr1, arm1, sep="");
	beg1 <- chrDict[i,3];
	end1 <- chrDict[i,4];
	chr2 <- chrDict[i,5];
	arm2 <- as.vector(chrDict[i,6]);
	chrArm2 <- paste(chr2, arm2, sep="");
	beg2 <- chrDict[i,7];
	end2 <- chrDict[i,8];
	seg <- chrDict[i,9];
    if (action=="sections") {
		bpStart1 <- chrDict[i,10];
		bpEnd1 <- chrDict[i,11];
		CytoStart1 <- as.vector(chrDict[i,12]);
		CytoEnd1 <- as.vector(chrDict[i,13]);
		bpStart2 <- chrDict[i,14];
		bpEnd2 <- chrDict[i,15];
		CytoStart2 <- as.vector(chrDict[i,16]);
		CytoEnd2 <- as.vector(chrDict[i,17]);
    }
	print(paste("On chromosome ", chrArm1,"_",chrArm2, " segment ", seg, sep=""));
	...
	dataFile <- paste(param, "_", dim, "D_", dataSet, "_", chrArm1,"_",chrArm2, "_seg", seg,".txt", sep="");
    ...
    outlier<-paste("out_",chrArm1,"_",chrArm2, sep="");
    ...
    if (action=="sections") {
		pvals <- rbind(pvals, c(chrArm1, chrArm2, dim, seg, phen1num_noOut, phen2num_noOut, beg1, end1, beg2, end2, bpStart1, bpEnd1, bpStart2, bpEnd2, CytoStart1, CytoEnd1, CytoStart2, CytoEnd2, pval, stat, sumPos, sumNeg));
    } else {
        pvals <- rbind(pvals, c(chrArm1, chrArm2, dim, seg, phen1num_noOut, phen2num_noOut, beg1, end1, beg2, end2, pval, stat, sumPos, sumNeg));
    }
...
if (action=="sections") {
    colnames(pvals) <- c("ChrArm1", "ChrArm2", "Dimension", "Segment", "TestNum", "CtrlNum", "Idx0.Beg1", "Idx0.End1","Idx0.Beg2", "Idx0.End2", "bpStart1", "bpEnd1","bpStart2", "bpEnd2", "CytoStart1", "CytoEnd1","CytoStart2", "CytoEnd2","P.Value", "Test.Stat", "AreaPos", "AreaNeg");
} else {
    colnames(pvals) <- c("ChrArm1", "ChrArm2", "Dimension", "Segment", "TestNum", "CtrlNum", "Idx0.Beg1", "Idx0.End1","Idx0.Beg2", "Idx0.End2","P.Value", "Test.Stat", "AreaPos", "AreaNeg");
}
